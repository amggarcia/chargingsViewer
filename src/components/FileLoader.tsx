...
function FileLoader(props: props) {
    ...
    async function loadXMLFile(files: FileList) {
        ...
        tempData.forEach(item => {
            if (Array.isArray(item.SubUsage))
                item.SubUsage = item.SubUsage;
            else if (item.SubUsage)
                item.SubUsage = [item.SubUsage];
            else
                item.SubUsage = []
    ...
    return (
        <div>
            <Button variant="contained" color="primary" onClick={onButtonClick} style={{ margin: "10px" }}>Load Files</Button>
            <input type='file'
                id='file'
                accept='.xml'
                ...
                style={{ display: 'none' }}
            />
            <Button variant="contained" color="secondary" onClick={clearFiles} style={{ margin: "10px" }}>Clear Files</Button>
            <Button variant="contained" style={{ margin: "10px" }} onClick={() => props.openAverages(true)}>Get averages</Button>
        </div >
    );
}
